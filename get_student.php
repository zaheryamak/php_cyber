...
if(!empty($_POST["classid"])) 
{
 ...
 ?><option value="">Sélectionner l'étudiant </option><?php
...
if(!empty($_POST["classid1"])) 
{
 ...
 while($row=$stmt->fetch(PDO::FETCH_ASSOC))
 {?>
  <p> <?php echo htmlentities($row['SubjectName']); ?><input type="text"  name="marks[]" value="" class="form-control" required="" placeholder="Indiquez les notes sur 20" autocomplete="off"></p>
...
<p>
<?php
echo "<span style='color:red'> Résultat déjà annoncé .</span>";
 echo "<script>$('#submit').prop('désactivé',true);</script>";
 ?></p>
<?php }
